...
//call to database for getting all posts
router.get("/", async (req, res) => {
  let all_posts = await Post.find().populate("author", "name -_id");
  res.send(all_posts);
});

//call to database for particular post
...
//call to database for getting userdetails
router.get("/:userid",auth, async (req,res,next) => {
try{
  var id=req.params.userid;
  const userdetail = User.find({_id:id},{'email':1});
  userdetail.exec()
  .then(data=>{
      res.status(200).json({
          message:"OK",
...
//call to database for creating a new post
...
//call to database for setting likes to the posts
